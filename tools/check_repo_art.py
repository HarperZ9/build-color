...
because a checkout that rewrote them would fail that comparison.
...
ART = ROOT / "docs" / "art"
SCHEMA = "build-color.repo-art/v1"
...
def check_render_is_deterministic(specs: list[Path]) -> list[str]:
    ...
    return [f"{p.name} renders differently every time" for p in specs if RENDER.rendered(p) != RENDER.rendered(p)]
...
def check_identity_per_repository(_unused: list[Path]) -> list[str]:
    ...
    marks = {n: ART_LIB.header_svg({"name": n, "role": "x", "tagline": "y", "words": ["z"]}) for n in names}
    bad = []
...
def check_seed_is_recorded(_unused: list[Path]) -> list[str]:
    ...
    svg = ART_LIB.header_svg({"name": "build-color", "role": "x", "tagline": "y", "words": []})
    stamp = f"SEED {ART_LIB.seed_for('build-color') % 100000:05d}"
        ...
        for flow in spec.get("flows", []):
            for edge in flow.get("returns", []):
                if edge["from"] // FLOW.PER_ROW != edge["to"] // FLOW.PER_ROW:
                    bad.append(f"return {edge['from']}->{edge['to']} crosses a row break")
    return bad
...
def check_every_illustration_is_shown(_unused: list[Path]) -> list[str]:
    ...
    haystack += "".join(p.read_text(encoding="utf-8", errors="ignore") for p in ROOT.glob("docs/**/*.md"))
    images = sorted(p for d in SHOWN_DIRS for p in (ROOT / d).glob("*") if p.suffix.lower() in {".svg", ".png"})
    return [f"committed but never shown: {_rel(p)}" for p in images if _rel(p) not in haystack]
...
def receipt() -> dict:
    specs = _specs()
    results = [
        {"name": name, "passed": not failures, "failures": failures}
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--json", action="store_true", help="emit the run as a receipt instead of a summary")
    ...
    for item in report["checks"]:
        ...
        for failure in item["failures"]:
            print(f"       {failure}")
    print(f"{len(report['outputs'])} files from {len(report['specs'])} spec files")
    return 0 if report["passed"] else 1
