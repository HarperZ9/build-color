...
def _hue_pair(rng: random.Random) -> tuple[str, str]:
    ...
    return (
        "#{:02X}{:02X}{:02X}".format(*(round(c * 255) for c in warm)),
        "#{:02X}{:02X}{:02X}".format(*(round(c * 255) for c in cool)),
    )
...
def _spokes(cx: float, cy: float, rng: random.Random, count: int = 300) -> str:
    ...
    for i in range(count):
        ...
        field = math.sin(angle * 3 + rng.random() * 0.3) * 0.22 + math.sin(angle * 7) * 0.13 + 0.68
    ...
    for level, segs in enumerate(buckets):
        if not segs:
            continue
        out.append(
            f'<path d="{"".join(segs)}" stroke="url(#ray)" '
...
def _rings(cx: float, cy: float, rng: random.Random) -> str:
    ...
    for ring in range(14):
        ...
        out.append(
            f'<circle cx="{_num(cx)}" cy="{_num(cy)}" r="{_num(radius)}" '
    ...
    for ring in range(1, 7):
        ...
        for k in range(n):
            ...
            if rng.random() > fall * 0.86 + 0.1:
                continue
            dots.append(
                f'<circle cx="{_num(cx + math.cos(angle) * radius)}" '
...
def _core(cx: float, cy: float, cool: str) -> str:
    ...
    """
    ticks = "".join(
        f'<path d="M{_num(cx + dx * 150)} {_num(cy + dy * 150)}L{_num(cx + dx * 160)} {_num(cy + dy * 160)}"/>'
