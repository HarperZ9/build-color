...
FACES = json.loads(Path(__file__).with_name("face-metrics.json").read_text(encoding="utf-8"))
...
BOUND = {
    ("sans", "regular"): CARD.SANS,
    ("sans", "bold"): CARD.SANS_BOLD,
    ("mono", "regular"): CARD.MONO_REG,
    ("mono", "bold"): CARD.MONO_BOLD,
}
...
def _cards() -> list[dict]:
    return [
        card
        for path in sorted(ART.glob("*.art.json"))
...
def _widest_face(text: str, role: str) -> tuple[str, float]:
    ...
    for name, face in FACES[group][weight].items():
        off = max(face)
        total = sum(face[ord(c) - FIRST] if FIRST <= ord(c) <= LAST else off for c in text)
...
def _over(text: str, role: str, budget: float, label: str) -> list[str]:
    ...
    if width <= budget:
        return []
    return [f"{label} draws {width:.0f}px into a {budget:.0f}px column in {name}: {text!r}"]


def _lines_that_run_long(text: str, label: str, role: str, budget: float, limit: int) -> list[str]:
...
def _card_overflows(card: dict) -> list[str]:
    ...
    for field in card["fields"]:
        ...
        bad += _over(field["value"], "value", VAL_BUDGET, f"{where}: the value on {field['key']}")
        bad += _lines_that_run_long(
            field["note"], f"{where}: the note on {field['key']}", "note", CARD.NOTE_BUDGET, CARD.NOTE_LINES
        )
    bad += _lines_that_run_long(card["footnote"], f"{where}: the footnote", "foot", CARD.FOOT_BUDGET, CARD.FOOT_LINES)
...
def text_that_overflows(cards: list[dict]) -> list[str]:
    ...
    for card in cards:
        ...
        if len(heads) != 3:
            bad.append(f"{card['file']} names {len(heads)} columns, and the drawing has three")
        for head, budget in zip(heads, HEAD_BUDGETS):
            bad += _over(head.upper(), "head", budget, f"{card['file']}: the {head!r} column head")
    return bad
...
def faces_the_renderer_underestimates(bounds: dict | None = None) -> list[str]:
    ...
    for (group, weight), bound in (bounds or BOUND).items():
        if len(bound) != LAST - FIRST + 1:
            bad.append(
                f"the {group} {weight} bound covers {len(bound)} characters and the faces cover {LAST - FIRST + 1}"
            )
            continue
        for name, face in FACES[group][weight].items():
            for i, thousandths in enumerate(face):
                if bound[i] < thousandths / 1000.0 - 1e-9:
                    bad.append(f"{name} draws {chr(FIRST + i)!r} wider than the renderer assumes it does")
    return bad
...
def characters_never_measured(cards: list[dict]) -> list[str]:
    ...
    for card in cards:
        ...
        off = sorted({c for text in drawn for c in text if not FIRST <= ord(c) <= LAST})
        if off:
            bad.append(f"{card['file']} draws {off!r}, and no face here was measured for it")
    return bad
    ...
    for card in cards:
        for field in card["fields"]:
            ...
            if DIGEST.search(value):
                bad.append(
                    f"{card['file']}: {field['key']} draws a digest, and a digest is true for one checkout: {value!r}"
                )
            if BIG_NUMBER.search(value):
                bad.append(f"{card['file']}: {field['key']} draws a number that moves with the commit: {value!r}")
    return bad
...
def alt_text_that_drifted(cards: list[dict]) -> list[str]:
    ...
    return [
        f"{card['file']}: the README describes it as something it is no "
        f"longer, because the spec alt is not the alt in the README"
        for card in cards
        if card["alt"] not in shown
    ]
...
def wrong_number_of_marks(cards: list[dict]) -> list[str]:
    ...
    bad = []
    for card in cards:
        hot = [f["key"] for f in card["fields"] if f.get("tone", "none") != "none"]
        if len(hot) != 1:
            bad.append(
                f"{card['file']} accents {len(hot)} rows, and one hot mark per view is the whole of the colour rule"
            )
    return bad
...
def checks() -> list[tuple]:
    ...
    return [
        ("art.card_draws_shapes_not_digits", lambda _unused: values_that_are_not_shapes(_cards())),
        ("art.card_text_fits_its_column", lambda _unused: text_that_overflows(_cards())),
        ("art.card_widths_bound_every_face", lambda _unused: faces_the_renderer_underestimates()),
        ("art.card_draws_measured_characters", lambda _unused: characters_never_measured(_cards())),
        ("art.card_carries_one_mark", lambda _unused: wrong_number_of_marks(_cards())),
        ("art.card_alt_reaches_the_readme", lambda _unused: alt_text_that_drifted(_cards())),
    ]
...
CONTROL = [
    {
        ...
        "fields": [
            {"key": "head", "value": "9f2c4ab71de0", "note": "ok", "tone": "verified"},
            {"key": "bytes", "value": "104857 bytes", "note": "ok", "tone": "drift"},
...
CONTROL_BOUND = {
    ("sans", "regular"): tuple(0.0 if i == ord("m") - FIRST else width for i, width in enumerate(CARD.SANS))
}
...
def control_failures() -> list[str]:
    ...
    return [
        f"the gate missed {what}"
        for caught, what in (
            (len(values_that_are_not_shapes(CONTROL)) == 2, "a digest and a byte count drawn as values"),
            (
            ...
            (
                len(faces_the_renderer_underestimates(CONTROL_BOUND)) == CONTROL_UNDERBOUNDS,
                "a renderer that assumes a lowercase m is no wider than average",
            ),
            (len(characters_never_measured(CONTROL)) == 1, "a character no face here was measured for"),
            (len(wrong_number_of_marks(CONTROL)) == 1, "a card wearing two hot marks"),
            (len(alt_text_that_drifted(CONTROL)) == 1, "a description that reaches no README at all"),
        )
        if not caught
    ]
