        ...
        for flow in spec.get("flows", []):
            ...
            for item in flow["outcomes"]:
                ...
                if len(item["note"]) > note_budget:
                    bad.append(f"the note under {item['label']} is wider than its box: {item['note']!r}")
    return bad
        ...
        "flows": [
            {
                ...
                "outcomes": [{"label": "OK", "note": "x" * 200}, {"label": "y" * 200, "note": "short"}],
            }
        ],
    }
]
...
def control_failures() -> list[str]:
    ...
    return [
        ...
        for caught, what in (
            ...
            (len(outcomes_that_overflow(CONTROL)) == 2, "an over-wide label and an over-long note"),
        )
        if not caught
    ]
