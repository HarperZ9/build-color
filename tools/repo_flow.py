...
def _card(index: int, stage: dict) -> str:
    ...
    notes = "".join(
        f'<text class="s" x="{_num(x + 14)}" y="{_num(y + 58 + i * 15)}">{_esc(line)}</text>'
        for i, line in enumerate(_wrap(stage.get("note", "")))
    )
    return (
        f'<g><rect class="card" x="{_num(x)}" y="{_num(y)}" '
...
def _forward(index: int) -> str:
    ...
    if y0 == y1:
        return f'<path class="fwd" marker-end="url(#af)" d="M{_num(x0 + w + 4)} {_num(mid0)}H{_num(x1 - 7)}"/>'
    ...
    return (
        f'<path class="fwd" marker-end="url(#af)" '
...
def _return(edge: dict) -> str:
    ...
    return (
        f'<path class="back" marker-end="url(#ab)" '
...
def _outcomes(items: list[dict], top: float, source: int) -> str:
    ...
    trunk = x0 + w0 / 2
    tone = {"verified": "var(--verified)", "drift": "var(--drift)", "none": "var(--muted)"}
    out = [f'<path class="thin" d="M{_num(trunk)} {_num(y0 + CARD_H)}V{_num(top - 24)}"/>']
