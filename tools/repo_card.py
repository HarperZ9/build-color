...
TONE = {"verified": "var(--verified)", "drift": "var(--drift)", "none": "var(--hairline)"}
...
def _widest(group: str, weight: str) -> tuple[float, ...]:
    ...
    return tuple(max(face[i] for face in faces) / 1000.0 for i in range(LAST - FIRST + 1))
...
def element_width(text: str, role: str) -> float:
    ...
    total = sum(table[ord(c) - FIRST] if FIRST <= ord(c) <= LAST else _OFF_TABLE for c in text)
...
def _wrap(text: str, width: float = NOTE_BUDGET, limit: int = NOTE_LINES) -> list[str]:
...
def _row(index: int, field: dict, row_h: float = ROW_H) -> str:
    ...
    notes = "".join(
        f'<text class="s" x="{_num(NOTE_X)}" y="{_num(y + 20 + i * LEADING)}">{_esc(line)}</text>'
        for i, line in enumerate(_wrap(field["note"]))
    )
    rule = (
        (f'<rect x="{_num(PAD)}" y="{_num(y)}" width="3" height="{_num(row_h - 8)}" fill="{tone}"/>') if accent else ""
    )
    return (
        f'<g><rect class="row" x="{_num(PAD)}" y="{_num(y)}" '
...
def _column_heads(labels: tuple[str, str, str] = HEADS) -> str:
    ...
    return "".join(
        f'<text class="k" x="{_num(x)}" y="{_num(TOP - 14)}" font-family="{MONO}">{_esc(label.upper())}</text>'
...
def _footnote(text: str, top: float) -> str:
    return "".join(
        f'<text class="s" x="{PAD}" y="{_num(top + i * FOOT_LEADING)}">{_esc(line)}</text>'
